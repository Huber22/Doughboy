...
Shader "Advanced Water Shader/WaterShader01 VR" {
    Properties {
        ...
        _ReflectionTex ("Reflection Tex", 2D) = "bump" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 sceneUVs = (i.projPos.xy / i.projPos.w) + (((float2((node_8565.r+node_1449.r),(node_8565.g+node_1449.g))*_SideRefractionPower)*(1.0 - node_163))+(node_9639.rg*_RefractionPower*node_3553));
...
////// Lighting:
                ...
                float node_9555 = (-0.1);
                float node_9385 = 1.0;
                float2 node_9089 = VRcode( sceneUVs.rg );
                float4 node_645 = tex2D(_ReflectionTex,TRANSFORM_TEX(node_9089, _ReflectionTex));
                float4 node_2278 = _Time;
                ...
                float3 finalColor = ((attenuation*_LightColor0.rgb)*((_Specular*pow(node_1603,exp2((_Gloss*10.0+1.0))))+saturate((1.0-(1.0-saturate((1.0-(1.0-(pow(saturate((node_9385 + ( (((max(0, LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sceneUVs)) - _ProjectionParams.g)-partZ)*_WaterDensity) - node_9555) * (0.0 - node_9385) ) / ((_WaterColor.rgb*8.0) - node_9555))),3.0)*sceneColor.rgb))*(1.0-((node_645.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelReflection)*_RelectionIntensity*node_163)+((1.0 - node_163)*(lerp(_WaterColor.rgb,dot(_WaterColor.rgb,float3(0.3,0.59,0.11)),(-2.0))*0.01)))))))*(1.0-(node_5933.rgb*node_2142.rgb*(1.0 - saturate((sceneZ-partZ)/_FoamDistance))*node_163*_FoamIntensity))))));
